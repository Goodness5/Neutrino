...
contract NeutrinoEstate is IERC721Receiver{
    ...
    function createFraction(address _nftContractAddress, uint256 _nftId, uint256 _supply) external returns(address) {
        ...
        FractionToken fractionToken = new FractionToken(_nftContractAddress, _nftId, msg.sender, _supply, "MyToken", "myt", address(this));
    ...
    function RetrievePropertyOnDefault(address _nftContractAddress, uint256 _nftId) external {
        ...
        require(FractionedERC20token.balanceOf(Registry[propIndex].isBuyer) != FractionedERC20token.totalSupply(), 'payment completed');
    ...
    function claimPaymentOnProperty(address _nftContractAddress, uint256 _nftId) external {
        ...
        require(FractionedERC20token.balanceOf(Registry[propertyIndex].isBuyer) == FractionedERC20token.totalSupply(), 'payment not complete');
    ...
    function exitProperty(address _nftContractAddress, uint256 _nftId, uint _status)external returns(uint[] memory, uint[] memory) {
        uint propertyIndex = PropertyNftIndex[_nftContractAddress][_nftId];
        ERC721URIStorage nftAddress = ERC721URIStorage(_nftContractAddress);
        FractionToken FractionedERC20token = FractionToken(Registry[propertyIndex].fractionContractAddress);
        require(_status == 0 || _status == 1, 'invalid option');
        if(_status == 0){
            require(Registry[propertyIndex].propertyStatus == Status.sale, "invalid status");
        }
        if(_status == 1){
            require(Registry[propertyIndex].propertyStatus == Status.rent, "invalid status");
        }
